...
local Steps = {
  ...
  -- ── 11 PLACES + FACTIONS + RADIO + PRISON (v0.25) ──
  { sec = '11 v0.25', id = 'V1', label = 'Radio powers up on channel 1', cmd = 'N with a radio in pockets', expect = 'lands on 1 with "Channel 1: emergency."; known list under the screen reads "known: 1"' },
  { sec = '11 v0.25', id = 'V2', label = 'Unknown channel refused', cmd = 'type 44, Enter', expect = '"Nothing on 44." with static; still on 1' },
  { sec = '11 v0.25', id = 'V3', label = 'Scan finds a live channel', cmd = 'S (or /radioscan)', expect = '4 s of hiss, then "New channel: 4" (or 7/13, or a channel a friend is on); ▲▼ now step 1 <-> 4' },
  { sec = '11 v0.25', id = 'V4', label = 'Joining a faction hands you its net', cmd = 'enlist at the gate', expect = '"New channel: 7 - Military Remnant net"' },
  { sec = '11 v0.25', id = 'F1', label = 'Zancudo gate post', cmd = 'F10 -> Saved locations -> Zancudo gate', expect = 'a Sergeant plus 5 marines at the gate; Enlist / Walk out / Armory / Talk on the Sergeant' },
  { sec = '11 v0.25', id = 'F2', label = 'Guards shoot the dead, not you', cmd = 'ob_zombie 5 near the gate; stand there', expect = 'marines open fire on the zombies and ignore you (unless you are a raider)' },
  { sec = '11 v0.25', id = 'F3', label = 'Boneyard post', cmd = 'F10 -> Saved locations -> Raider cache', expect = 'a Warlord plus 4 raiders; Join / Leave / Cache / Talk on the Warlord' },
  { sec = '11 v0.25', id = 'F4', label = 'Enlist works end to end', cmd = 'Sergeant -> Enlist -> confirm', expect = 'job military, channel 7 learned, F1 Faction row military. If nothing happens: jobs paste-in missing (04 -Only all)' },
  { sec = '11 v0.25', id = 'P1', label = 'Prison pool', cmd = 'console: ob_pool  then drive into Bolingbroke', expect = 'prints prison left=50; inside, zombies spawn (runners) but never more than are left' },
  { sec = '11 v0.25', id = 'P2', label = 'Pool counts down and clears', cmd = 'kill them (or console: ob_pool prison 3, then kill 3)', expect = 'ob_pool shows left falling; at 0 a radio line "somebody cleared it"' },
  { sec = '11 v0.25', id = 'P3', label = 'Prison opens when clear', cmd = 'cafeteria, infirmary, towers, cell block door', expect = 'before clear: no targets, claim refused "Not while they are inside"; after: cafeteria food, infirmary meds, tower safes with one guard on top, cells claimable' },
  { sec = '11 v0.25', id = 'M1', label = 'Map places', cmd = 'open the map', expect = 'named blips: 24/7 & LTD, clothing, barbers, mechanics, gun stores, vaults, medical stations, workbenches, Bolingbroke; no vanilla shop/bank blips (Renewed-Banking stopped)' },
  { sec = '11 v0.25', id = 'M2', label = 'Clothing store', cmd = 'Binco Strawberry', expect = '"Change clothes (free)" opens the appearance editor; "Loot the racks" gives sheets/a duffel/old money' },
  { sec = '11 v0.25', id = 'M3', label = 'Barber charges old money', cmd = 'any barber zone', expect = '"Haircut · 5 Old Money": refused under 5; takes 5 and opens the editor' },
